...
if [ "${TUTTI_AGENT_SKILLS_AUTO_UPDATE:-1}" = "0" ]; then
  exit 0
fi
...
MARKETPLACE_NAME="${TUTTI_AGENT_SKILLS_MARKETPLACE_NAME:-tutti-agent-skills}"
INTERVAL_SECONDS="${TUTTI_AGENT_SKILLS_UPDATE_INTERVAL_SECONDS:-86400}"
CACHE_ROOT="${XDG_CACHE_HOME:-${HOME:-/tmp}/.cache}"
STATE_DIR="$CACHE_ROOT/tutti-agent-skills"
STATE_FILE="$STATE_DIR/last-claude-plugin-update"
