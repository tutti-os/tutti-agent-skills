...
Agent app main flows must resolve provider choices through Tutti workspace-app scoped `preferences/agent`, `agent-providers/status`, and `agent-providers/{provider}/composer-options` APIs and follow `references/dynamic-agent-providers.md`. Do not replace that catalog with app-local allowlists or kit runtime provider lists. Show every returned provider, keep unavailable providers disabled with a reason, choose a usable default, use the `@tutti-os/agent-acp-kit/tutti` catalog helpers when available, and keep only a thin whole-catalog compatibility fallback that shows Codex and Claude with `default` models.
